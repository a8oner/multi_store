...
class SupplieRegister extends StatefulWidget {
  const SupplieRegister({super.key});

  @override
  State<SupplieRegister> createState() => _SupplieRegisterState();
}

class _SupplieRegisterState extends State<SupplieRegister> {
  ...
  CollectionReference
      suppliers = //ref ile ilgilidir .ref de kaydedilecek dosyaya path açar
        ...
        try {
          ...
          firebase_storage.Reference ref1 =
              firebase_storage.FirebaseStorage.instance.ref(
                  'supp-images/$email.jpg'); //mail her bir kullanıcı için farklı olduğu için email kullanıldı
          ...
          storeLogo = await ref1.getDownloadURL();
          suppliers.doc(_uid).set({
            'storeName': storeName,
            'email': email,
            'storeLogo': storeLogo,
            'phone': '',
            'sid': _uid, //supplier id
            'coverimage': '',
          });
          /*
          .add({
          ...
          */
          ...
          //Storage ve firebase database aktif edildi video 50. storage:images için firestore database belge için
